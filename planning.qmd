...
still actively maintained. GRIM and GRIMMER test whether reported means
and standard deviations are possible at all given the sample size and
the granularity of the data, and SPRITE constructs hypothetical samples
compatible with the reported summary statistics and the bounds of the
response scale. Further checks compare numbers across a paper, for
instance whether subgroup sizes add up to the stated sample size. Such
checks identify results that are inconsistent or impossible, which are
worth raising with the original authors, but a flag can also stem from
rounding or transcription slips and does not by itself establish that a
finding is wrong, just as passing these checks does not guarantee that
it will replicate. The same techniques are introduced under
[Uncertainty](choosing_study.qmd#uncertainty) as an aid to choosing a
target; here they form the right-hand path of @fig-repr-decision-tree.
