...
Neuroimaging research faces unique challenges for reproducibility and
replication due to high-dimensional MRI data and numerous methodological
choices. Preprocessing and analysis pipelines offer countless parameter
options, leading to variability in results even when analyzing the same
dataset. Mass-univariate approaches generate a vast number of
statistical tests, requiring careful control for false positives and
flexible definitions of regions of interest. Sample size justification
is complicated by spatially correlated effect distributions, and many
studies remain underpowered given typically small effects. Defining
replication success demands attention to the three-dimensional
distribution of effects, with multiple potential spatial criteria and
limitations. Open science practices, including automated pipelines,
transparent scripts, preregistration, and data sharing, are essential
yet underutilized. Their implementation is further complicated by
privacy concerns and sustainability considerations. Despite challenges,
rigorous replication and transparency remain crucial for reliable
neuroimaging findings.

*After this chapter, you will know:*

-   Why does MRI research have more “researcher degrees of freedom” than
    many other scientific fields, and how can this affect replication?

-   How do small sample sizes and complex data structures influence the
    reliability of neuroimaging results?

-   What strategies can improve reproducibility in neuroimaging, and why
    are open science practices particularly important in this field?
:::
...
(i.e., significant) results [@Eklund2016, @Ioannidis2005]. Most of these factors are
arguably aggravated in MRI studies, making replication research in this
field particularly relevant albeit challenging. In addition, the
analyzed data and obtained findings are characterized by a
three-dimensional spatial component (or four dimensions in case of
functional MRI studies (fMRI) in combination with time series data),
which further complicates the matter. In the following we summarize the
inherent peculiarities of replication research in the field of
neuroimaging.
...
all parameters [@ZhouEtAl2022], and that different teams analyzing the
same dataset can arrive at different final conclusions dependent on the
used pipeline [@BotvinikNezerEtAl2020]. These findings emphasize the need for standardized and containerized preprocessing pipelines (for a further discussion see 9.0.5). Furthermore, a large variety of operationalizations of neurobiological targets is available. For
example, cerebral gray matter structure could be investigated as
...
distribution of effect sizes around a peak localization. These characteristics make conventional power calculations difficult to implement and have contributed to the discontinuation of several early neuroimaging power analysis tools (e.g., https://jeanettemumford.org/fmripower/).


To address these challenges, simulation-based power analysis approaches have been previously
...
tools have been discontinued. One example for such a simulation-based tool is fmrism [@Ellis2020]. As an alternative to voxel-wise power estimation, @Durnez2016 proposed estimating power based on the prevalence of active peaks, an approach with more favorable statistical properties that facilitates practical sample size estimation. The complexity outlined above has limited the practical applicability of many proposed tools, several of which have since been discontinued. Therefore, the utilization of power anal-ysis is extremely rare in MRI research despite all the endeavors made. 

Given these limitations, continued methodological development is needed to make power analysis more practical and applicable for neuroimaging studies. 
...
neuroimaging modalities [below 2% explained variance, @MarekEtAl2022;
@WinterEtAl2022] and could require thousands of individuals to obtain
robust and replicable statistical estimates [@MarekEtAl2022]. In
...
them likely underpowered [@ButtonEtAl2013]. Smaller samples may be
suitable however, for research questions where the neurobiological
effect sizes are expected to be larger, such as in psychosis research or
when using highly individually tailored or within-subject designs
[@LynchEtAl2024; @MarekEtAl2022; @RosenbergFinn2022; @SpisakEtAl2023].

## Criteria for Replication Success
...
with an effect size for each spatial unit (e.g., voxel).
@GoltermannAltegoer2025 describe a variety of potential criteria
focusing on statistical significance in accordance with different
spatial definitions revolving around the original finding. These include
significance either at the peak voxel location (where the effect in the
original study had the largest effect size), or in a ROI that can be
defined in terms of spatial proximity to this peak voxel (for example a
...
the brain, as some anatomically or functionally defined structures may
vary in size and distinctiveness (e.g., comparing the small and
clearly-defined amygdala with a large and difficult to define
dorsolateral prefrontal cortex). Thus, it may be necessary to combine
several criteria in a systematic and/or subjective manner.
...
highly advisable [e.g., @EstebanEtAl2019], ideally in combination with
containerized toolbox environments for preprocessing and analysis
[@RentonEtAl2024]. In face of reproducibility challenges the transparent
...
[@JwaEtAl2024; @AbramianEklund2019]. When the publication of raw data is
not possible, comprehensive statistical brain maps (i.e., the
statistical results in each voxel) should be made publicly available in
non-thresholded form [@TaylorEtAl2023] and/or data can be published in
aggregated form (e.g., summarized for one brain region).
...
@PuhlmannEtAl2025 for a perspective].
